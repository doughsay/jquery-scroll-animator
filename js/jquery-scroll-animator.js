var ScrollAnimator = function(definitions) {
	...
	function onScroll() {
		...
		var position = w.scrollTop() / height;
		
		var c = {};
		c.r = Math.round(255 * position);
		c.g = Math.round(128 * position);
		c.b = Math.round(0 * position);
		
		var d = {};
		d.r = Math.round(64 * (position-1)*-1);
		d.g = Math.round(64 * (position-1)*-1);
		d.b = Math.round(255 * (position-1)*-1);
